...
class LocalNodeModel {
  ...
  async _doResolvePrepareNodesQueue(type) {
    ...
    const {
      ...
      return {
        queryFields: (0, _merge2.default)(queryFields, nextQueryFields),
        fieldsToResolve: (0, _merge2.default)(fieldsToResolve, nextFieldsToResolve)
      };
    }, {
      queryFields: {},
      fieldsToResolve: {}
    ...
    if (!(0, _isEmpty2.default)(actualFieldsToResolve)) {
      const {
        schemaCustomization: {
      ...
      // @ts-ignore
      this._preparedNodesCache.set(typeName, (0, _merge2.default)({},
      // @ts-ignore
...
const getQueryFields = ({
  ...
  return (0, _merge2.default)(filterFields, ...sortFields.map(pathToObject), ...group.map(pathToObject), ...distinct.map(pathToObject), ...max.map(pathToObject), ...min.map(pathToObject), ...sum.map(pathToObject));
};
...
const dropQueryOperators = filter => Object.keys(filter).reduce((acc, key) => {
  ...
  const v = value[k];
  if ((0, _isPlainObject2.default)(value) && (0, _isPlainObject2.default)(v)) {
...
async function resolveRecursive(nodeModel, schemaComposer, schema, node, type, queryFields, fieldsToResolve, customContext) {
  ...
  for (const fieldName of Object.keys(fieldsToResolve)) {
    ...
    if (gqlField && innerValue != null) {
      if ((0, _graphql.isCompositeType)(gqlFieldType) && !(gqlNonNullType instanceof _graphql.GraphQLList)) {
        innerValue = await resolveRecursive(nodeModel, schemaComposer, schema, innerValue, gqlFieldType, queryField, (0, _isObject2.default)(fieldToResolve) ? fieldToResolve : queryField, customContext);
      } else if ((0, _graphql.isCompositeType)(gqlFieldType) && ((0, _isArray2.default)(innerValue) || innerValue instanceof _iterable.GatsbyIterable) && gqlNonNullType instanceof _graphql.GraphQLList) {
        innerValue = await Promise.all(innerValue.map(item => item == null ? item : resolveRecursive(nodeModel, schemaComposer, schema, item, gqlFieldType, queryField, (0, _isObject2.default)(fieldToResolve) ? fieldToResolve : queryField, customContext)));
      }
    }
    if (innerValue != null) {
  ...
  // @ts-ignore
  return (0, _pickBy2.default)(resolvedFields, (value, key) => queryFields[key]);
}
...
const determineResolvableFields = (schemaComposer, schema, type, fields, isNestedAndParentNeedsResolve = false) => {
  ...
  Object.keys(fields).forEach(fieldName => {
    ...
    if ((0, _isObject2.default)(field) && gqlField) {
      ...
      if (!(0, _isEmpty2.default)(innerResolved)) {
...
const deepObjectDifference = (from, to) => {
  ...
  Object.keys(from).forEach(key => {
    ...
    if (toValue) {
      if ((0, _isPlainObject2.default)(toValue)) {
        const deepResult = deepObjectDifference(from[key], toValue);
        if (!(0, _isEmpty2.default)(deepResult)) {
