...
function link(options = {
  ...
  function linkResolverValue(fieldValue, args, context, info) {
    ...
    if (options.by === `id`) {
      if (Array.isArray(fieldValue)) {
        ...
        const id = fieldValue !== null && fieldValue !== void 0 && fieldValue.id || typeof fieldValue === `string` ? fieldValue : undefined;
  ...
  function linkResolverQueryResult(fieldValue, queryResult, returnType) {
    if (returnType instanceof _graphql.GraphQLList && Array.isArray(fieldValue) && Array.isArray(queryResult)) {
      return fieldValue.map(value => queryResult.find(obj => (0, _getValueAt.getValueAt)(obj, options.by, {
        keepNonMatches: false
      }) === value));
    } else {
      return queryResult;
...
const defaultFieldResolver = exports.defaultFieldResolver = function defaultFieldResolver(source, args, context, info) {
  if (typeof source == `object` && source !== null || typeof source === `function`) {
    if (info.from) {
      const val = (0, _getValueAt.getValueAt)(source, info.from, {
...
const defaultResolver = exports.defaultResolver = wrappingResolver(defaultFieldResolver);
