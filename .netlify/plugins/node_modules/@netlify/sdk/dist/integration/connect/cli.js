...
`);
        ...
        ${replaceTemplateDataString(templateFile, templateData)
            .split(`\n`)
            .map((line) => {
...
export async function writeStackbitDevFiles({ connectorDir, integrationConfig, connectorConfig, }) {
    ...
    if (!localFrontendConfig) {
        ntliLog(`This connector supports Netlify Create but no local frontend configuration was found in integration.yaml\nLocal Netlify Create will not run.

To run a local server and test your connector, follow this example integration.yaml
