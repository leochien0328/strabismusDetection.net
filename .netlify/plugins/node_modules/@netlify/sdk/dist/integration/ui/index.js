            ...
            surfaces: this._surfaces.map((surface) => {
                ...
                Object.values(surface.routes).forEach((route) => {
                    ...
                    route.onLoadCallbacks.forEach((callback) => {
                        surfaceScript += generateCallback(`WFUI_LOAD_${route.path}`, callback.toString());
                    });
                    Object.entries(route.callbacks).forEach(([id, callback]) => {
                        ...
                        surfaceScript += generateCallback(sanitisedId, callback.toString());
                    });
                    Object.entries(route.formCallbacks).forEach(([id, callback]) => {
                        if (id.endsWith(":submit")) {
                            surfaceScript += generateCallback(`WFUI_SAVE_${id}`, callback.toString());
                        }
                        else if (id.endsWith(":cancel")) {
                            surfaceScript += generateCallback(`WFUI_CANCEL_${id}`, callback.toString());
                        }
                    });
                    Object.entries(route.interactiveCallbacks).forEach(([id, callback]) => {
                        surfaceScript += generateInteractiveCallback(`WFUI_INTERACTION_${id}`, callback.toString());
                    });
                });
                return {
